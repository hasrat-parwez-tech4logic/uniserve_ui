...
const formattedUsers = computed(() => {
  return users.value.map(user => ({
    ...
    user_type: user.sessionCount > 5 ? 'ACTIVE' : user.sessionCount > 1 ? 'RETURNING' : 'NEW',
...
const userStats = computed(() => {
  ...
  const avgDuration = userSessions.value.length > 0 ? Math.round(totalDuration / userSessions.value.length) : 0;
  ...
  return {
    ...
    avgDuration,
    totalSteps,
    avgStepsPerSession: userSessions.value.length > 0 ? Math.round(totalSteps / userSessions.value.length) : 0
  };
});

const goBack = () => {
  if (project.value?.organizationId) {
    const orgId = typeof project.value.organizationId === 'object' 
    ...
    if (project.value) {
      ...
      if (response.success) {
        ...
        stats.value = {
          ...
          totalSessions: response.data.reduce((sum, u) => sum + (u.sessionCount || 0), 0),
          totalMessages: response.data.reduce((sum, u) => sum + (u.messageCount || 0), 0),
          avgSessionDuration: 0,
  ...
  try {
    ...
    if (sessionData.success) {
      ...
      const flowPromises = sessionData.data.map(async (session) => {
        try {
          const flowResponse = await fetch(`${API_URL}/sessions/${session._id}/flows`, {
            headers: {
              'Authorization': `Bearer ${localStorage.getItem('token')}`
            }
          });
...
onMounted(async () => {
  ...
  try {
    ...
    if (!project.value || project.value._id !== projectId.value) {
      ...
      const storedProject = projectStore.projects.find(p => p._id === projectId.value);
      if (storedProject) {
  ...
  <div class="flex flex-col h-full bg-gray-50">
    ...
    <div class="bg-white border-b border-gray-200 px-6 py-5">
      <div class="flex items-center gap-4 mb-3">
        ...
        <div class="flex-1">
          <div class="flex items-center gap-3">
            <h1 v-if="project" class="text-2xl font-bold text-gray-900">{{ project.name }}</h1>
            <span v-if="project" :class="['px-3 py-1 rounded-full text-xs font-medium', project.status === 'ACTIVE' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-700']">
              {{ project.status }}
            </span>
          </div>
        </div>
      ...
      <div v-if="project" class="flex items-center gap-6 text-sm">
        ...
        <div v-if="project.organizationId" class="flex items-center gap-2">
          ...
          <span class="text-gray-600">{{ typeof project.organizationId === 'object' ? project.organizationId.name : 'Organization' }}</span>
        </div>
        <div class="flex items-center gap-2">
          <Calendar :size="16" class="text-gray-500" />
          <span class="text-gray-600">Created {{ new Date(project.createdAt).toLocaleDateString() }}</span>
        </div>
      </div>
    </div>
    ...
    <div class="flex-1 overflow-hidden">
      ...
      <div v-if="activeView === 'users'" class="h-full flex gap-6 p-6">
        ...
        <div :class="selectedUser ? 'w-2/5' : 'w-full'" class="bg-white rounded-xl shadow-sm border border-gray-200 flex flex-col transition-all">
          <div class="p-4 border-b border-gray-200">
            <h2 class="text-lg font-bold text-gray-900">Project Users</h2>
            <p class="text-sm text-gray-600 mt-1">{{ users.length }} users total</p>
          </div>
        ...
        <div v-if="selectedUser" class="w-3/5 bg-white rounded-xl shadow-sm border border-gray-200 flex flex-col overflow-hidden">
          <!-- User Journey View -->
          ...
          <div class="bg-gray-50 p-6 border-b border-gray-200">
            <div class="flex items-center gap-4">
              ...
              <div class="flex-1">
                <h2 class="text-xl font-bold text-gray-900 mb-1">{{ selectedUser.name || 'Unknown User' }}</h2>
            ...
            <div class="grid grid-cols-3 gap-4 mt-4">
              <div class="bg-white rounded-lg p-3 text-center border border-gray-200">
                ...
                <p class="text-lg font-bold text-gray-900">{{ selectedUser.sessionCount }}</p>
              </div>
              <div class="bg-white rounded-lg p-3 text-center border border-gray-200">
                <p class="text-xs text-gray-500 mb-1">Messages</p>
                <p class="text-lg font-bold text-gray-900">{{ selectedUser.messageCount }}</p>
              </div>
              <div class="bg-white rounded-lg p-3 text-center border border-gray-200">
                <p class="text-xs text-gray-500 mb-1">Last Active</p>
                <p class="text-xs font-semibold text-gray-900">{{ formatRelativeTime(selectedUser.lastActive) }}</p>
              </div>
            </div>
          </div>
          ...
          <div class="flex-1 overflow-y-auto p-6">
            <div v-if="userSessionsLoading" class="flex items-center justify-center h-full">
              <div class="text-center">
                <RefreshCw :size="48" class="animate-spin text-blue-500 mx-auto mb-4" />
            ...
            <div v-else-if="userSessions.length === 0" class="flex items-center justify-center h-full">
              <div class="text-center">
                <Activity :size="64" class="text-gray-300 mx-auto mb-4" />
                <p class="text-gray-600 text-lg mb-2">No sessions found</p>
                <p class="text-gray-500 text-sm">This user hasn't had any sessions yet</p>
              </div>
            </div>
            ...
            <div v-else class="space-y-4">
              ...
              <SessionJourneyBlock
                ...
                :key="session.id"
                :session="session"
                :flows="sessionFlows.filter(f => f.session_id === session.id)"
              />
            </div>
          </div>
        </div>
        ...
        <div class="max-w-7xl mx-auto">
          ...
          <div class="grid grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-8">
              <h3 class="text-xl font-bold text-gray-900 mb-6">Analytics Dashboard</h3>
              <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div class="bg-gray-50 rounded-lg p-8 border-2 border-dashed border-gray-300">
                  <div class="text-center">
                    ...
                    <p class="text-gray-600 font-medium text-lg">Sessions Over Time</p>
                    <p class="text-sm text-gray-500 mt-2">Line chart placeholder</p>
                  </div>
                </div>
                <div class="bg-gray-50 rounded-lg p-8 border-2 border-dashed border-gray-300">
                  <div class="text-center">
                    <Activity :size="64" class="text-gray-400 mx-auto mb-4" />
                    <p class="text-gray-600 font-medium text-lg">User Engagement</p>
                    <p class="text-sm text-gray-500 mt-2">Bar chart placeholder</p>
                  </div>
                </div>
                <div class="bg-gray-50 rounded-lg p-8 border-2 border-dashed border-gray-300">
                  <div class="text-center">
                    <TrendingUp :size="64" class="text-gray-400 mx-auto mb-4" />
                    <p class="text-gray-600 font-medium text-lg">Conversion Funnel</p>
                    <p class="text-sm text-gray-500 mt-2">Funnel chart placeholder</p>
                  </div>
                </div>
                <div class="bg-gray-50 rounded-lg p-8 border-2 border-dashed border-gray-300">
                  <div class="text-center">
                    <MessageSquare :size="64" class="text-gray-400 mx-auto mb-4" />
                    <p class="text-gray-600 font-medium text-lg">Message Volume</p>
                    <p class="text-sm text-gray-500 mt-2">Area chart placeholder</p>
                  </div>
                </div>
              </div>
            </div>
